...
if [ "$WORDPRESS_MULTISITE" = "true" ]; then
    if $(wp-cli core is-installed --allow-root); then
        ...
        wp-cli core multisite-install --url="${VIRTUAL_HOST:-localhost}" --title="BCGov Dev Multi" --admin_user=${WORDPRESS_USER:-admin} --admin_password="${WORDPRESS_PASSWORD:-password}" --admin_email="admin@example.com" --allow-root
        echo >&2 "Creating Multisite ${VIRTUAL_HOST:-localhost}"
    fi
else
    if ! $(wp-cli core is-installed --allow-root); then
        wp-cli core install --url="${VIRTUAL_HOST:-localhost}" --title="BCGov Dev Single" --admin_user=${WORDPRESS_USER:-admin} --admin_password="${WORDPRESS_PASSWORD:-password}" --admin_email="admin@example.com" --allow-root
